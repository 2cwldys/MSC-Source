...
#define IsDead (pmove->deadflag == DEAD_DEAD)
...
================
*/
void PM_MS_SimpleCheckVel( int id )
{
	/*int i;
	for (i=0 ; i<3 ; i++)
	{
		if (pmove->velocity[i] > pmove->movevars->maxvelocity)
			LogText( "[%i]Vel %i == %f\r\n", id, i, pmove->velocity[i] );
	}*/
}
...
//
	for (i=0 ; i<3 ; i++)
	{
		...
		if (pmove->velocity[i] > pmove->movevars->maxvelocity) 
		{
			//LogText( "[ERROR!] PlayerMove Err Code: %i (%f)\r\n", g_PMErrCode, pmove->velocity[i] );
...
//
			...
			((pmove->onground == -1) || (pmove->friction != 1)) )	// relfect player velocity
		{
			...
			VectorCopy( new_velocity, original_velocity );
			PM_MS_SimpleCheckVel( 40000 );
		}
		else
		{
			for (i=0 ; i<numplanes ; i++)
			{
				PM_MS_SimpleCheckVel( 40025 );
				PM_ClipVelocity (
					original_velocity,
					planes[i],
					pmove->velocity,
					1);
				PM_MS_SimpleCheckVel( 40050 );
	...
	//LogText( "vec3_origin[1] == %f\r\n", vec3_origin[1] );
	PM_MS_SimpleCheckVel( 49000 );
	return blocked;
}
...
// first try just moving to the destination	
	...
	//LogText( "Vel Before FlyMove: %f\r\n", pmove->velocity[1] );
	// Slide move
	clip = PM_FlyMove ();

	PM_MS_SimpleCheckVel( 300 );
...
void PM_PlayerMove ( qboolean server )
{
	...
	if ( !IsDead && !(pmove->flags & FL_ONTRAIN) )
	{
		pLadder = PM_Ladder();
		if ( pLadder )
		{
...
void PM_Move ( struct playermove_s *ppmove, int server )
{
	...
	//MUST have the following line.  Somewhere, my monster movement code
	//causes the engine to corrupt vec3_origin which MUST be 0,0,0
	//So make sure its 0,0,0 every time
	vec3_origin[0] = vec3_origin[1] = vec3_origin[2] = 0;
	PM_PlayerMove( ( server != 0 ) ? true : false );

	if( !server )
		//pmove->Con_Printf("%.2f %.2f", pmove->origin[0], pmove->origin[1] );
		pmove->Con_Printf("%.2f %.2f %.2f\n", pmove->velocity[0], pmove->velocity[1], pmove->time );
