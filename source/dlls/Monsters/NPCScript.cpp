	...
	*/
    ...
    else if( Cmd.Name() == "skilllevel" )
	{
        ...
        { 
			...
			foreach( p , m_PlayerDamage.size() )  
            {
				foreach( r, SKILL_MAX_ATTACK )
				{
					foreach( s, STATPROP_ALL_TOTAL )
	...
	//************************** CATCHSPEECH ********************
		...
		{	//Parameters: <script event> <trigger1> [trigger2] [trigger3] [etc.]
			...
			foreach( i, Params.size()-1 )	//Starts at parameter 2
		...
		if( m_MenuCurrentOptions )
		{
			...
			foreach( i, Menuoptions.size( ) )
				if( MenuOption.Priority > Menuoptions[i].Priority )
				{
					...
					foreach( r, (Menuoptions.size( )-1) - i )
					{
	...
	//******************************* TAKEDMG ****************************
		...
		if( Params.size() >= 2 )
		{
			...
			else
			{
				...
				//char Debug_Type[32];
				//float Debug_AMT = 0.0;
				foreach( i, m.TakeDamageModifiers.size() )
				{
			...
			if( fAttack )
			{
				...
				if( pProjectile )
				{
					...
					if( Params.size() >= 6 )
					{
						...
						{
							vStartPos = pev->origin + GetRelativePos( pev->angles, StringToVec( Params[5] ) ); //x = right-left, y = forward-back, z = up-down
						}
					}
					...
					pProjectile->TossProjectile( this, vStartPos, vForward * flRange, flDamage );
				}
			}
		}
		else ERROR_MISSING_PARMS;
			...
			if ( IsPlayer() )
			{
				if ( !thoth_instat.contains(".") )
				{
					...
					if( pStat )
					{
						...
						foreach( i, pStat->m_SubStats.size() )
						{
			...
			if( pTarget && pTarget->IsPlayer() )
			{
				...
				else if( Params[0] == "trade" )
				{
					if( Params.size() >= 5 )
					{
						...
						if( pPlayer->Storage_GetStorage( StorageName ) )
						{
							pPlayer->Storage_Open( Params[4], StorageName, atof(Params[3]), entityinfo_t(this) );
							CallScriptEvent( Callback + "_success" );
						}
						else
...
bool CMSMonster::GetScriptVar( msstring &ParserName, msstringlist &Params, CScript *BaseScript, msstring &Return )
{
		...
		if( ParserName == "$cansee" && Params.size() >= 1 )
		{
			...
			foreach( i, m_EnemyListNum ) SightedList.add( (CBaseEntity *)m_hEnemyList[i] );

			foreach( i, SightedList.size() )
			{
