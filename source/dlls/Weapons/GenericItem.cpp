...
bool CGenericItem::CanWearItem( )
{
	...
	if( m_pPlayer )
	{
		foreach( iloc, m_WearPositions.size() )
		{
			wearpos_t *pPlayerPos = NULL;
			foreach( ploc, m_pPlayer->m_WearPositions.size() )
			{
			...
			foreach( i, m_pOwner->Gear.size() )
			{
				...
				foreach( iwloc, pItemWorn->m_WearPositions.size() )
				{
	...
	//****************************** WEARABLE **************************
		...
		if( Params.size( ) >= 1 )
		{
			...
			else if( Params.size( ) >= 2 )
			{
				...
				foreach( i, Positions.size() )
				{
					msstring &PosName = Positions[i];

					bool Exists = false;
					foreach( i, m_WearPositions.size() )
					{
						if( PosName != m_WearPositions[i].Name )
							continue;
...
CGenericItem *FindParryWeapon( CMSMonster *pMonster, /*out*/int &iPlayerHand, /*out*/int &iAttackNum )
{
	...
	foreach( i, MAX_PLAYER_HANDS )
	{
		...
		foreach( a, pHandItem->m_Attacks.size() )
		{
...
{
	SendGenericItem( pPlayer, (genericitem_full_t &)genericitem_full_t(pItem), fNewMessage );
}
...
{
	foreach( e, MSCLGlobals::m_ClEntites.size() )
	{
