...
{ \
	if( Prop == name ) return (Return = VecToString( Vector(color.r,color.g,color.b) ));  \
...
void CScript::CLScriptedEffect( msstringlist &Params )
{
	...
	if( Params[0] == "tempent" )
	{
		...
		if( Params[1] == "sprite" || Params[1] == "model" )
		{
			...
			int Spot = 0;
			foreach( i, ARRAYSIZE(g_TempEntExtra) )
	...
	void	( *Callback_TempEntPlaySound )( TEMPENTITY *pTemp, float damp ) )
{
	...
	if ( !*ppTempEntActive )
	{
		...
		{
			foreach( i, ARRAYSIZE(g_TempEntExtra) )				//On level change, this is called.  Clear all tempent extra data from last level
