		...
		if (j >= m_nCachedBones)
		{
			...
			if (pbones[i].parent == -1) 
			{
				if( FBitSet(Ent.curstate.playerclass,ENT_EFFECT_FOLLOW_ROTATE) )
				{
					...
					foreach( c, 3 ) HostMatrix[c][3] = OwnerOrg[c];
					...
					foreach( c, 3 ) AttachmentMatrix[c][3] = Ent.origin[c];
...
//Master Sword - Rendering the view model actually means rendering two separate models
	if( FBitSet(m_pCurrentEntity->curstate.oldbuttons, MSRDR_VIEWMODEL ) )
	{
		...
		foreach( hand, MAX_PLAYER_HANDITEMS )	//Includes playerhands view model
		{
		...
		foreach( m, m_pmodel->nummesh )
		{
			...
			while( ptricmds[0] )
			{
...

				{
					...
					Modifycmds += (2 * 4);
					foreach( Triangle, NumTriangleCmds-2 )
					{				
